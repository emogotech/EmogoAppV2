 ...
 */
    ...
    func prepareSearchBar(){
      //  txtSearch = UITextField(frame: CGRect(x: 0, y: 0, width: kFrame.size.width - 135, height: 30))
        txtSearch = UITextField(frame: CGRect(x: 0, y: 0, width: kFrame.size.width, height: 30))
                ...
                if kRefreshCell {
                    if let index = selectedIndexPath {
                        if index.row == indexPath.row {
                            cell.viewContent.fadeIn(0.2, delay: 0.4) { (_) in
                            }
                            kRefreshCell = false
                        }
                    }
...
//        })
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        if (textField.text?.trim().isEmpty)! {
         //   self.txtSearch.frame = CGRect(x: 0, y: 0, width: kFrame.size.width - 135, height: 30)
         //   self.txtSearch.frame = CGRect(x: 0, y: 0, width: kFrame.size.width - 80, height: 30)
