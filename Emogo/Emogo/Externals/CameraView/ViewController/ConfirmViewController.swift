		...
		if let asset = asset {
			var fetcher = SingleImageFetcher()
				.onSuccess { [weak self] image in
                    self?.hideSpinner(spinner)
                    self?.enable()
                    let cropController = CropViewController(croppingStyle: (self?.croppingStyle)!, image: image)
            ...
            hideSpinner(spinner)
            enable()
            let cropController = CropViewController(croppingStyle: self.croppingStyle, image: newImage)
            cropController.delegate = self
            self.present(cropController, animated: true, completion: nil)
		}
	}
...
extension UIImage {
	...
	func fixOrientation() -> UIImage {
     ...
     func rotateCameraImageToProperOrientation() -> UIImage? {
        ...
        if (width > kFrame.size.width || height >  kFrame.size.width) {
            
            scaleRatio = min( kFrame.size.width / bounds.size.width, kFrame.size.width / bounds.size.height)
