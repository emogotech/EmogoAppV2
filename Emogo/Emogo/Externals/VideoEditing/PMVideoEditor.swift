...
class PMVideoEditor: NSObject {
     ...
     *  @return finish: (URL?, Error?)-> Void) (required). This will call when video will be finished processing or any error occur while procerssing.
     */
    func setVideoRate(path: URL, rate: Float, begin: Float64, end: CMTime, progress: @escaping (Float,String)-> Void, finish: @escaping (URL?, Error?)-> Void) {
        self.progressBlock = progress
        self.finishedBlock = finish
        self.extract_Audio_Video_Assets(path: path)
        guard let composition = self.getComposition(withDuration: CMTimeMakeWithSeconds(begin, Int32(NSEC_PER_SEC)), endDuration: end, shouldScale: true, rate: rate) else {
            finish(nil, NSError(domain: "Invalid asset", code: -999, userInfo: nil))
            return
        }
        let video_composition = self.renderContents(layers: [], fps: video_Track.minFrameDuration, composition: composition)
    ...
    private func extract_Audio_Video_Assets(path: URL) {
        ...
        video_Track = asset.tracks(withMediaType: AVMediaType.video).first!
        audio_Track = asset.tracks(withMediaType: AVMediaType.audio).first!
    }
    private func getComposition(withDuration beginDuration: CMTime, endDuration: CMTime, shouldScale: Bool, rate: Float) -> AVMutableComposition? {
        ...
        do {
            ...
            if video_Track != nil {
                let video_composition_Track: AVMutableCompositionTrack = composition.addMutableTrack(withMediaType: AVMediaType.video, preferredTrackID: kCMPersistentTrackID_Invalid)!
                if shouldScale {
            ...
            if audio_Track != nil {
                let audio_composition_Track: AVMutableCompositionTrack = composition.addMutableTrack(withMediaType: AVMediaType.audio, preferredTrackID: kCMPersistentTrackID_Invalid)!
                if shouldScale {
    ...
    private func beginTimer() {
        ...
        exportTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(self.exportTimerCall), userInfo: nil, repeats: true)
    }
    ...
    func getFileDuration(file:URL?) -> (Float?,CMTime?) {
        if let url = file {
