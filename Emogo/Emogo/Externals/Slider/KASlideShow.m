...
- (void) populateImageView:(UIImageView*) imageView andIndex:(NSUInteger) index
{
    ...
    } else if ([dataObj isKindOfClass:[NSURL class]])
    {
        ...
        [imageView sd_setImageWithURL:imageURL placeholderImage:[UIImage imageNamed:@"stream-card-placeholder"]];
...
{
    if ([self.delegate respondsToSelector:@selector(slideShowDidSelect:)]) {
        [self.delegate slideShowDidSelect:self];
    }
    /*
