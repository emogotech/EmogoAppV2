...
open class FSPagerView: UIView,UICollectionViewDataSource,UICollectionViewDelegate {
    ...
    open var isAddBackground: Bool = true {
        didSet {
            self.configurebackGround()
        }
    }
    
    open var isAddTitle: Bool = true {
        didSet {
    ...
    public func scrollViewWillEndDragging(_ scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
        ...
        guard let function = self.delegate?.pagerViewDidEndScrollAnimation else {
            return
        }
        function(self)
    ...
    func configureAnchorInLabel(){
        self.lblCurrentType.bottomAnchor.constraint(equalTo: self.bottomAnchor, constant: -15).isActive = true
    ...
    func configurebackGround(){
        ...
        imgView.tag = 999898
        if self.viewWithTag(999898) != nil {
            imgView.removeFromSuperview()
        }
        if self.isAddBackground == false {
            imgView.image = nil
