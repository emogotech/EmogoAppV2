...
open class FSPagerViewCell: UICollectionViewCell {
    ...
    open  var currentIndexValue : Int?
    ...
    func addLayerInImageView(isTrue : Bool){
        if isTrue {
            ...
            zoomInAndOut.speed = 0.5
            
            self.imageView?.layer.add(zoomInAndOut, forKey: nil)
        }
        else{
            self.imageView?.widthAnchor.constraint(lessThanOrEqualToConstant: 40).isActive = true
            self.imageView?.heightAnchor.constraint(lessThanOrEqualToConstant: 40)  .isActive = true
