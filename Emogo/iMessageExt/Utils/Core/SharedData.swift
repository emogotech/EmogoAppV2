...
class SharedData: NSObject {
    ...
    func getPhoneCode(){
        ...
        print(cc ?? "")
        if cc == nil {
            let url = URL(string: "http://freegeoip.net/json/")
            ...
            let dataTask = URLSession.shared.dataTask(with: request) {
                ...
                do {
                    if let jsonResult = try JSONSerialization.jsonObject(with: data!, options: []) as? [String:Any] {
                        let strCode:String = self.getCountryCallingCode(countryRegionCode: jsonResult["country_code"] as! String)
                        print(strCode)
                    }
